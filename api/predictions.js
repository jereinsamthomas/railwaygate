...
export default async function handler(req, res) {
  ...
  // Summary counts
  const counts = {OPEN:0, APPROACHING:0, CLOSING:0, CLOSED:0};
  for (const p of predictions) counts[p.status_code] = (counts[p.status_code]||0)+1;
  ...
  res.status(200).json({
    ...
    summary: counts,
    traffic_source: tomtomKey ? 'tomtom_live' : 'simulated',
