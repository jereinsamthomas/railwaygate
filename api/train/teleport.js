...
export default function handler(req, res) {
  ...
  // Acknowledge teleport - actual position is time-based so returns current state
  const train = getTrainState();
  res.status(200).json({
    ok: true, train: { lat: train.lat, lon: train.lon, speed: train.speed, direction: train.direction }
  });
}
