...
export default async function handler(req, res) {
  ...
  res.status(200).json({
    stations: STATIONS,
    gates: GATES,
    waypoints: STATIONS.map(s => [s.lat, s.lon]),
    train: primaryTrain ? {
      train_number: primaryTrain.train_number,
      name: primaryTrain.name,
      lat: primaryTrain.lat,
      lon: primaryTrain.lon,
      speed: primaryTrain.speed,
      direction: primaryTrain.direction,
      heading_deg: primaryTrain.heading_deg,
    } : null,
    all_trains: trains,
    active_train_count: trains.length,
    timestamp: new Date().toISOString(),
    source: 'live_timetable_kinematics'
  });
}
